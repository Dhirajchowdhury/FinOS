  ...
  const handleSendMessage = async ({
    ...
    try {
      ...
      const stateResponse = await analysisApi.runAnalysis({
        request: message,
        context: selectedAgentId ? { selected_agent_id: selectedAgentId } : {},
      });
      ...
      const errorMsg: ConversationMessage = {
        id: `err-${Date.now()}`,
        sender: "finos",
        text: err?.message || "An error occurred while communicating with the FinOS 10-agent engine. Please ensure you are logged in.",
        timestamp: "Just now",
        agentsConsulted: ["report"],
      };
